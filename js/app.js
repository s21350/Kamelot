(() => {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".reviews__swiper")) new core(".reviews__swiper", {
            ...
            if (document.querySelector(".our-projects__swiper")) new core(".our-projects__swiper", {
                modules: [ Navigation, Pagination, EffectFade ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
        ...
        if ($openFilter) document.getElementById("open-filter").onclick = function() {
        ...
        if ($closeFilter) document.getElementById("close-filter__btn").onclick = function() {
        ...
        if ($wrapper) $wrapper.innerHTML = createTemplates(products);
        function filterProducts(products, filters, priceMin, priceMax, wrapper) {
            let filterProducts = products;
            if (filters) {
                const filtersName = Object.keys(filters);
                for (let i = 0; i < filtersName.length; i++) if (filters[filtersName[i]].length > 0) filterProducts = filterProducts.filter((item => filters[filtersName[i]].includes(item[filtersName[i]])));
            }
            if (priceMin && priceMax) filterProducts = filterProducts.filter((item => Number(item.price) >= Number(priceMin) && Number(item.price) <= Number(priceMax)));
        ...
        function applyFilter(form) {
            ...
            form.addEventListener("submit", (e => {
                ...
                const $checboxBrand = form.querySelector(".filter-brand").querySelectorAll("input[type=checkbox]:checked");
                const $checboxColor = form.querySelector(".filter-color").querySelectorAll("input[type=checkbox]:checked");
                const $checboxType = form.querySelector(".filter-material").querySelectorAll("input[type=checkbox]:checked");
                for (let i = 0; i < $checboxBrand.length; i++) brands.push($checboxBrand[i].value);
                for (let i = 0; i < $checboxColor.length; i++) colors.push($checboxColor[i].value);
                for (let i = 0; i < $checboxType.length; i++) types.push($checboxType[i].value);
                const filters = {};
                if (brands.length >= 1) filters.brand = brands;
                if (colors.length >= 1) filters.color = colors;
                if (types.length >= 1) filters.type = types;
